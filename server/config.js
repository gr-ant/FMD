// App config store (key/value JSON in the `configs` table).
import { pool } from './db.js'

export function registerConfigRoutes(app) {
  app.get('/api/_config', async (req, res) => {
    try {
      const { rows } = await pool.query('SELECT key, value FROM configs ORDER BY key')
      const out = {}
  ...
  app.get('/api/_config/:key', async (req, res) => {
    try {
      const { rows } = await pool.query('SELECT value FROM configs WHERE key = $1', [req.params.key])
  ...
  app.delete('/api/_config/:key', async (req, res) => {
    try { await pool.query('DELETE FROM configs WHERE key = $1', [req.params.key]); res.json({ ok: true }) }
    catch (e) { res.status(500).json({ error: String(e) }) }
  })
}
