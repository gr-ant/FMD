...
export function evalArith(expr: unknown, record: FmdRecord): number {
  try {
...
export function parseAggregate(expr: unknown): Aggregate | null {
...
export function splitEntityField(
  ref: unknown,
  sources: Set<string> | Map<string, unknown>,
): { source: string; field: string | null } {
...
export function aggregate(fn: string, rows: FmdRecord[], field: string | null): number {
  rows = rows || []
